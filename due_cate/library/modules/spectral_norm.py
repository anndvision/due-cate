...
class SpectralNormFC(spectral_norm.SpectralNorm):
    ...
    def apply(
        ...
        with torch.no_grad():
            ...
            u = functional.normalize(
                weight.new_empty(h).normal_(0, 1), dim=0, eps=fn.eps
            )
            v = functional.normalize(
                weight.new_empty(w).normal_(0, 1), dim=0, eps=fn.eps
            )
...
class SpectralNormConv(spectral_norm.SpectralNorm):
    def compute_weight(self, module, do_power_iteration: bool) -> torch.Tensor:
        ...
        if do_power_iteration:
            with torch.no_grad():
                for _ in range(self.n_power_iterations):
                    v_s = functional.conv_transpose2d(
                    ...
                    v = functional.normalize(v_s.view(-1), dim=0, eps=self.eps, out=v)

                    u_s = functional.conv2d(
                    ...
                    u = functional.normalize(u_s.view(-1), dim=0, eps=self.eps, out=u)
                if self.n_power_iterations > 0:
                    # See above on why we need to clone
                    u = u.clone(memory_format=torch.contiguous_format)
                    v = v.clone(memory_format=torch.contiguous_format)

        weight_v = functional.conv2d(
    ...
    def apply(module, coeff, input_dim, name, n_power_iterations, eps):
        ...
        with torch.no_grad():
            ...
            v = functional.normalize(torch.randn(num_input_dim), dim=0, eps=fn.eps)
            ...
            u = functional.normalize(torch.randn(num_output_dim), dim=0, eps=fn.eps)
