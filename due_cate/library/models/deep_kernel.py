...
from due_cate.library.models import core
from due_cate.library.modules import dense
from due_cate.library.modules import convolution
from due_cate.library.modules import gaussian_process
    ...
    def predict_mus(self, ds, batch_size=None):
        ...
        with torch.no_grad():
            for batch in dl:
                ...
                samples = samples.t()
                mus = samples.chunk(2, dim=0)
                mu_0.append(mus[0])
                mu_1.append(mus[1])
        return (
            torch.cat(mu_0, 0).to("cpu").numpy().transpose(),
            torch.cat(mu_1, 0).to("cpu").numpy().transpose(),
        )
